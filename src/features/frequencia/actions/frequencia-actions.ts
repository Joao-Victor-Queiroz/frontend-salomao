...
export async function atualizarFrequencia(id: string, data: { status: 'P' | 'FNJ' | 'FJ'; justificativa?: string | null }) {
    try {
        const api = await apiAxios();
        const response = await api.patch(`frequencia/${id}`, data);
        return { success: true, message: 'Frequência atualizada com sucesso!', data: response.data}
    } catch (error: unknown) {
        let errorMessage = 'Erro desconhecido';
